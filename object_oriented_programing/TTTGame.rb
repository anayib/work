class Board
  WINNING_LINES = [[1, 2, 3], [4, 5, 6], [7, 8, 9]] + # winnig rows
                  [[1, 4, 7], [2, 5, 8], [3, 6, 9]] + # winning cols
                  [[1, 5, 9], [3, 5, 7]] #..............wining diagonals
  ...
  def reset
    (1..9).each {|k,v| @squares[k] = Square.new}
  end
...
class TTTGame
  ...
  def human_moves
    ...
    loop do
      ...
      board[square] = human.marker #board.set_square_at(square, human.marker)
  end

  def computer_moves
    board[board.unmarked_keys.sample]= computer.marker# <- same . board.[]=(board.unmarked_keys.sample, computer.marker)
  end
